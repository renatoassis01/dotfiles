...
if [ -x "$(command -v apt)" ]; then
    sudo apt update && sudo apt install -y curl zsh stow kitty unzip wget xclip make postgresql-client shellcheck pspg
fi

# Arch likes
if [ -x "$(command -v pacman)" ]; then
   sudo pacman -S stow zsh kitty unzip wget terminator xclip make shellcheck pspg
fi
